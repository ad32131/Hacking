...
#--------------------------------------------------
# You need to modify the core.py file with UTF8
#--------------------------------------------------
# C:\Python27\lib\site-packages\frida\core.py
#--------------------------------------------------
#import codecs
...
def on_messages_from_js(message, data):
  if message['type'] == 'send':
    if type(message['payload']) == dict:
      with open(message['payload']['file'], 'wb') as f:
...
js_script = """
    ...
    */
    ...
    Interceptor.replace(_open, new NativeCallback(function(path, flags, mode) {
        /*
        address = Module.findExportByName(null, 'LoadEngine');
        if (address !== null) {
    ...
    */
    ...
    _socket = getExportFunction("f", "socket", "int", ["int", "int", "int"]);
    ...
    _connect = getExportFunction("f", "connect", "int", ["int", "pointer", "int"]);
    ...
    _recv = getExportFunction("f", "recv", "int", ["int", "pointer", "int", "int"]);
    ...
    */
    ...
    Interceptor.attach(Module.findExportByName(null, "dlsym"), {
        onEnter: function(args) {
            ...
            {
                address = Module.findExportByName(null, 'Java_com_xshield_da_z');
...
print('[+] spawn')
pid = device.spawn(["com.velic.mts.vex"])
print('[+] attach')
process = device.attach(pid)
# device = frida.get_device_manager().enumerate_devices()[-1]
# process = device.attach("com.velic.mts.vex")
print('[+] create_script')
script = process.create_script(js_script)
script.on('message', on_messages_from_js)
script.load()
print('[+] resume')
device.resume(pid)
sys.stdin.read()
